...
export default function JobKategoriList() {
    const [kategori, setKategori] = useState([]);

    const getKategori = async () => {
        const response = await axios.get('http://localhost:3000/talenthub/api/v1/job_categories');
    ...
    const deleteKategori = async (id) => {
        try {
            await axios.delete(`http://localhost:3000/talenthub/api/v1/job_categories/${id}`);
            getKategori();
                ...
                <table className="mt-4 min-w-full border border-gray-300 bg-white rounded-lg overflow-hidden">
                    <thead className="bg-gray-100">
                        <tr>
                            ...
                            <th className="px-4 py-2 text-left text-gray-700 font-semibold border-b">Kategori</th>
                    ...
                    <tbody>
                        {kategori.map((kategori, index) => (
                            <tr key={kategori.id} className="hover:bg-gray-50 transition">
                                <td className="px-4 py-2 border-b">{index + 1}</td>
                                <td className="px-4 py-2 border-b">{kategori.name}</td>
                                <td className="px-4 py-2 border-b text-center space-x-2">
                                    <Link
                                        to={`/admin/JobKategori/edit/${kategori.category_id}`}
                                    ...
                                    <button
                                        onClick={() => deleteKategori(kategori.category_id)}
